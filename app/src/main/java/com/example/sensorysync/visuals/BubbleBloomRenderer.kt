...
class BubbleBloomRenderer {
    ...
    private fun renderGazeReticleSmall(
        ...
        val reticleColor = if (isTargetingBubble) Color(0xFF00E676) else baseColor
        val radius = (if (isTargetingBubble) 6.5f else 4.2f) * markerScale

        drawScope.drawCircle(
            color = reticleColor.copy(alpha = (if (isTargetingBubble) 0.65f else 0.35f) * markerOpacity),
        ...
        drawScope.drawCircle(
            ...
            radius = (if (isTargetingBubble) 2.0f else 1.3f) * markerScale,
            center = Offset(gazeX, gazeY)
        )
