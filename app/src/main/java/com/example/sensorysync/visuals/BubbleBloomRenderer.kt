...
class BubbleBloomRenderer {
    ...
    fun render(drawScope: DrawScope, state: ControlState, frameDeltaTime: Float) {
        ...
        if (state.gazeData.isFaceDetected) {
            renderGazeReticleSmall(drawScope, gazePx, gazePy, activeFocusedIndex != null)
        }
    }
    ...
    private fun renderGazeReticleSmall(
        ...
        val reticleColor = if (isTargetingBubble) Color(0xFF00E676) else Color(0xFF00E5FF)
        val radius = if (isTargetingBubble) 6.5f else 4.2f

        drawScope.drawCircle(
            color = reticleColor.copy(alpha = if (isTargetingBubble) 0.65f else 0.30f),
            radius = radius + 3.0f,
        ...
        drawScope.drawCircle(
            ...
            radius = if (isTargetingBubble) 2.0f else 1.3f,
            center = Offset(gazeX, gazeY)
        )
